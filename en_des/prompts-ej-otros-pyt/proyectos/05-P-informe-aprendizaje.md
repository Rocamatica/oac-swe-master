...
Ejecuta stage-management-system/prompts/proyectos/05-P-informe-aprendizaje.md para Capturar aprendizaje sobre el desarrollo (centrado en conocimiento adquirido, no en tareas/bitácora ejecutada) en el repo sobre [tema concreto]
DIR_DESTINO: DIR
...
```
[DIR_PYT]/aprendizaje-[tema-kebab-case]-[YYYY-MM-DD].md
```

El DIR_PYT se determina así (por orden de prioridad):
1. Si el usuario especificó `DIR_DESTINO: [ruta]` → esa ruta
2. Si no, y hay un proyecto activo → se deduce automáticamente de la sesión
3. Si no → `temp/`

Si no hay proyecto activo, listar los existentes en `stage-development-system/` y preguntar al usuario cuál usar.
...
**Rama:** [nombre de rama]
**Archivo:** [DIR_PYT]/aprendizaje-[tema]-[YYYY-MM-DD].md
